class Api::V1::MagicLinksController < ApplicationController
  ...
  def request_magic_link
    ...
    return render json: { error: "Something went wrong. Please try again." }, status: :bad_request unless magic_link_result[:success]
  ...
  def verify
    magic_link_service = MagicLinkService.new
    result = magic_link_service.verify_magic_token_and_create_access_token(email: user.email, magic_token: params[:token])
    return render json: { error: result[:error] }, status: :unauthorized unless result[:success]

    render json: {
      message: 'Successfully logged in',
      user: result[:user].as_json(only: [:id, :email, :display_name, :first_name, :last_name]),
